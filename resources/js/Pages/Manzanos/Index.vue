...
const accionesRow = () => {
    // editar
    $("#table-manzano").on("click", "button.editar", function (e) {
        e.preventDefault();
        let id = $(this).attr("data-id");
        axios.get(route("manzanos.show", id)).then((response) => {
            setManzano(response.data);
            accion_dialog.value = 1;
            open_dialog.value = true;
        });
    });
    // eliminar
    $("#table-manzano").on("click", "button.eliminar", function (e) {
        e.preventDefault();
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                let respuesta = await deleteManzano(id);
...
<template>
    <Head title="Manzanos"></Head>

    <!-- BEGIN breadcrumb -->
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="javascript:;">Inicio</a></li>
        <li class="breadcrumb-item active">Manzanos</li>
    </ol>
    <!-- END breadcrumb -->
    <!-- BEGIN page-header -->
    <h1 class="page-header">Manzanos</h1>
    <!-- END page-header -->
